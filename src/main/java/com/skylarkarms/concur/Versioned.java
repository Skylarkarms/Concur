...
public record Versioned<T>(
        ...
        T value)
{
    public boolean isDiff(T that) {
        return !Objects.equals(value, that);
    }

    public boolean isDiff(Versioned<T> that) {
        return isDiff(that.value);
    }

    public<M> M applyToVal(Function<T, M> map) {
        return map.apply(value);
    }

    static final Versioned<?> DEFAULT = new Versioned<>(0, null);
    @SuppressWarnings("unchecked")
    public static<T> Versioned<T> getDefault() {
        return (Versioned<T>) DEFAULT;
    }
    ...
    public boolean isNewerThan(int thatVersion) {
    ...
    public boolean isNewerThanVersion(Versioned<?> that) {
     ...
     * */
    public <M> Versioned<M> apply(Function<T, M> map) {
        return new Versioned<>(version, map.apply(value));
    }

    /**Will swap the value of this container while keeping the same {@code version}*/
    public <M> Versioned<M> swapType(M newValue) {
        return new Versioned<>(version, newValue);
    }
     ...
     * */
    public Versioned<T> newValue(T t) {
        return new Versioned<>(version + 1, t);
    }
